...
import numba.experimental, warnings
...
def produceAdjList(edgeList: list[tuple[Any, ...]]) -> dict[Any, list[Any]]:
    adjlist: dict[Any, list[Any]] = defaultdict(list)
    for tup in edgeList:
        adjlist[tup[0]].append(tup[1])
    return adjlist

"""
def convertgraph(adjMat: list[list[int]], vertexes: list[Any]) -> dict[Any, list[Any]]:
    adjlist: dict[Any, list[Any]] = defaultdict(list)
...
def DijkstraAlgo(graph: dict[Any, dict[Any, int]], current: Any) -> dict[Any, int|float]:
...
def bellmanfordAlgo(graph: dict[Any, list[Any]]) -> dict[Any, int|float]:
    pass
...
if __name__ == "__main__":
    ...
    adjlist: dict[int, list[int]] = {vertex+1: adjVertex for vertex, adjVertex in enumerate(g)}
    print(adjlist)

    vertex: list[str] = ['A', 'B', 'C']
    edges: list[tuple[str, ...]] = [('A', 'C'), ('A', 'B'), ('B', 'A'), ('C', 'B')]
    newlist: dict[str, list[str]] = defaultdict(list)
    for tup in edges:
        newlist[tup[0]].append(tup[1])
